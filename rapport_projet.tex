...
\begin{itemize}
    ...
    \item \textbf{Webapp} : accessible sur \texttt{http://localhost:8081}, permet de lancer/arrêter la pipeline (simulateur + bridge + Spark) via des boutons HTMX.
...
La webapp contrôle les conteneurs via deux mécanismes :

\begin{enumerate}
    \item \textbf{Docker SDK Python} (\texttt{docker.from\_env()}) : le socket Docker (\texttt{/var/run/docker.sock}) est monté dans le conteneur webapp. Cela permet de lister, démarrer et arrêter les conteneurs simulateur et mqtt-bridge.
    \item \textbf{Docker exec} via le SDK : pour lancer le \texttt{spark-submit} dans le conteneur \texttt{spark-master}, la webapp utilise \texttt{container.exec\_run()}. Le Spark job est détaché (\texttt{detach=True}) pour ne pas bloquer la réponse HTTP.
\end{enumerate}

Contrainte de sécurité : le socket Docker est monté en lecture-écriture. Dans un environnement de production, il faudrait restreindre les permissions ou utiliser une API Docker sécurisée.
...
\begin{itemize}
    \item \textbf{Lister les machines} : tableau des machines configurées avec leurs paramètres (température cible, vibration cible, alpha).
...
\begin{itemize}
    ...
    \item \textbf{Configuration multi-machines} : le simulateur publie sur les topics hiérarchiques pour pipe-101 et pump-303. Les deux machines sont visibles dans le flux Kafka.
