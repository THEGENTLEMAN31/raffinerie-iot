...
\begin{itemize}
    \item $\alpha_{\text{temp}} = 0.05$ : inertie forte. Le temps de convergence à 95\% est $t_{95} = \ln(0.05) / \ln(1 - \alpha) \approx 58$ itérations ($\sim$116~s). La température met environ 2~minutes à atteindre une nouvelle consigne.
    \item $\alpha_{\text{vib}} = 0.2$ : inertie modérée. $t_{95} \approx 13$ itérations ($\sim$26~s). Les vibrations réagissent plus vite aux changements mécaniques.
    \item $\varepsilon_{\text{temp}} \sim \mathcal{U}(-0.2, 0.2)$ : bruit de $\pm 0.2$°C, représentant les fluctuations thermiques ambiantes.
    \item $\varepsilon_{\text{vib}} \sim \mathcal{U}(-0.05, 0.05)$ : bruit de $\pm 0.05$~mm/s, les vibrations mécaniques étant naturellement plus stables.
\end{itemize}

Cette formule modélise à la fois l'inertie thermique d'un échangeur et l'inertie mécanique d'une pompe.

\subsubsection{Code de la boucle de simulation}

\begin{lstlisting}[language=Python,caption={Noyau de la simulation inertielle}]
ALPHA_TEMP, ALPHA_VIB = 0.05, 0.2
TEMP_CIBLE, VIB_CIBLE = 95.0, 1.2  # mode normal

while True:
    scenario = random.choices(["NORMAL","SURCHAUFFE","USURE"],
                              weights=[0.9, 0.05, 0.05])[0]
    if scenario == "SURCHAUFFE": TEMP_CIBLE = 145.0
    elif scenario == "USURE":    VIB_CIBLE = 4.5
    else:                        TEMP_CIBLE, VIB_CIBLE = 95.0, 1.2

    for _ in range(random.randint(30, 60)):
        temp = temp * (1 - ALPHA_TEMP) + TEMP_CIBLE * ALPHA_TEMP \
               + random.uniform(-0.2, 0.2)
        vib  = vib  * (1 - ALPHA_VIB)  + VIB_CIBLE  * ALPHA_VIB  \
               + random.uniform(-0.05, 0.05)
        temp, vib = clamp(temp, 31, 149), clamp(vib, 0.1, 4.9)
        publier_mqtt(temp, vib)
        time.sleep(2)
\end{lstlisting}
...
Les données sont uploadées sur MinIO au format JSON Lines via l'API S3 :

\begin{lstlisting}[language=Python,caption={Génération dataset labeling}]
with open("training_data.json", "w") as f:
    for i in range(5000):
        scenario = random.choices(
            ["NORMAL", "SURCHAUFFE", "USURE"],
            weights=[0.90, 0.05, 0.05]
        )[0]
        cible_temp = 95.0 if scenario != "SURCHAUFFE" else 145.0
        cible_vib  = 1.2 if scenario != "USURE" else 4.5
        # Meme logique inertielle que le simulateur temps reel
        temp = temp * (1 - 0.05) + cible_temp * 0.05 + random.uniform(-0.2, 0.2)
        vib  = vib  * (1 - 0.20) + cible_vib  * 0.20 + random.uniform(-0.05, 0.05)
        label = 1 if scenario != "NORMAL" else 0
        for capteur, valeur in [("temperature", temp), ("vibration", vib)]:
            f.write(json.dumps({"valeur": valeur, "label": label, ...}) + "\n")
\end{lstlisting}

Chaque itération produit deux lignes (température + vibration) avec le même label, portant le total à 10\,000 enregistrements.
...
\begin{enumerate}
    ...
    \item \textbf{Lags temporels} ($\times 5$) : $V_{t-1}, V_{t-2}, V_{t-3}, V_{t-4}, V_{t-5}$ — soit une fenêtre de 10~secondes (5 pas $\times$ 2~s). Au-delà de 5 lags, l'information apportée par des pas plus anciens devient négligeable (le coefficient de corrélation partielle avec $V_{t-6}$ est $<0.1$).
    \item \textbf{Pentes} ($\times 2$) : $\text{slope\_1} = V_t - V_{t-1}$ (pente immédiate), $\text{slope\_3} = (V_t - V_{t-3}) / 3$ (tendance sur 6~s). Ces pentes sont les \textbf{features les plus importantes} pour la détection précoce : une valeur encore normale mais en hausse produit un score d'anomalie élevé. Dans le modèle final, \texttt{slope\_1\_temperature} a une importance de 0.31 (31\% du poids de décision), contre 0.08 pour la valeur instantanée.
    \item \textbf{Type de capteur} ($\times 1$) : encodé via \texttt{StringIndexer} (temperature $\rightarrow$ 0.0, vibration $\rightarrow$ 1.0).
\end{enumerate}

Les lags sont calculés par fenêtre de partition Spark en utilisant la clause \texttt{Window} :

\begin{lstlisting}[language=Python,caption={Calcul des lags et pentes dans Spark}]
fenetre = Window.partitionBy("machine_id", "type_capteur") \
                .orderBy("timestamp")

for k in range(1, 6):
    df = df.withColumn(f"v_lag{k}",
                       lag("valeur", k).over(fenetre))

df = df.withColumn("slope_1", col("valeur") - col("v_lag1"))
df = df.withColumn("slope_3", (col("valeur") - col("v_lag3")) / 3)
\end{lstlisting}

Pendant l'entraînement, les lignes avec des lags nuls (début de séquence, pas d'historique) sont supprimées via \texttt{dropna()}. En inférence temps réel, ces lignes sont conservées en remplaçant les lags nuls par la valeur courante via \texttt{coalesce(col("v\_lag\_k"), col("valeur"))} — la pente devient nulle, évitant un faux positif au démarrage du streaming.
...
\begin{itemize}
    ...
    \item \textbf{Robustesse au déséquilibre de classe} (91\% / 9\%) : le bagging (échantillonnage avec remise) et le vote basé sur les distributions de feuilles gèrent le déséquilibre sans nécessiter de sur-échantillonnage ou de pondération explicite.
    \item \textbf{Interprétabilité} : l'importance des features est accessible via \texttt{model.featureImportances}, et on peut expliquer pourquoi une prédiction est positive (pente anormale). Pas de boîte noire.
...
\subsubsection{Calcul de la probabilité dans Spark ML}

Spark ML ne fait pas un vote majoritaire sec ($k$ arbres sur 30 votent anomalie). Chaque arbre stocke, dans chaque feuille, la distribution des classes observée pendant l'entraînement. Lors de l'inférence :

\begin{enumerate}
    \item La feature vector traverse chaque arbre jusqu'à une feuille.
    \item La feuille contient un vecteur de comptes : $[n_{\text{normal}}, n_{\text{anomalie}}]$ (nombre d'échantillons d'entraînement tombés dans cette feuille).
    \item L'arbre produit la distribution normalisée : $[n_{\text{normal}} / (n_{\text{normal}} + n_{\text{anomalie}}),\; n_{\text{anomalie}} / (n_{\text{normal}} + n_{\text{anomalie}})]$.
    \item Le score final est la moyenne des 30 distributions : $\text{probability} = \frac{1}{30} \sum_{i=1}^{30} \text{distribution}_i$.
\end{enumerate}

\noindent\textbf{Conséquence directe} : même en régime normal, chaque feuille contient un petit résidu d'échantillons anormaux (le dataset d'entraînement avait 8.6\% d'anomalies, incompressible par le bagging). Une feuille dans une région \og normale \fg{} de l'espace des features peut contenir $[97, 3]$ (3 anomalies sur 100), produisant une probabilité de $0.03$ pour cet arbre. La moyenne des 30 arbres donne $\sim$0.026 — ce n'est ni un risque ni un vote minoritaire, c'est la \textbf{composition résiduelle des feuilles} héritée des données d'entraînement.

\begin{lstlisting}[language=Python,caption={Extraction de la probabilité dans le code Spark (source interne MLlib simplifié)}]
# Chaque noeud feuille stocke les statistiques d'entraînement
class LeafNode:
    def predict(self, features):
        total = self.stats.sum  # nombre d'echantillons dans cette feuille
        # Retourne la proportion de chaque classe
        return [self.stats.classCounts[0] / total,
                self.stats.classCounts[1] / total]

# La foret moyenne les distributions
class RandomForestClassificationModel:
    def predictProbability(self, features):
        probs = [tree.rootNode.predict(features) for tree in self.trees]
        return sum(probs) / len(probs)  # moyenne des 30 distributions
\end{lstlisting}
...
L'AUC (Area Under the ROC Curve) mesure la capacité du modèle à séparer les deux classes, indépendamment du seuil. Un AUC de 0.80 signifie qu'il y a 80\% de chances que le modèle classe correctement une paire (normal, anomalie) aléatoire. La matrice de confusion au seuil 0.5 donne :

\begin{center}
\begin{tabular}{|l|c|c|}
\hline
 & \textbf{Prédit Normal} & \textbf{Prédit Anomalie} \\
\hline
\textbf{Réel Normal} & 1758 (VN) & 112 (FP) \\
\textbf{Réel Anomalie} & 85 (FN) & 102 (VP) \\
\hline
\end{tabular}
\end{center}

Soit un recall (taux de vrais positifs) de 54.5\% et une précision de 47.7\% au seuil 0.5. Ce score modeste s'explique par le déséquilibre des classes (91\% normal) : le modèle maximise l'AUC en étant conservateur, car prédire \og normal \fg{} est statistiquement rentable. En production, le seuil est abaissé à 0.4 pour le passage au jaune (sensibilité accrue) et à 0.7 pour l'alerte rouge (spécificité maximale).
...
\textbf{Importances des features} (extraites du modèle entraîné) :
...
\hline
slope\_1 (pente immédiate) & 0.31 \\
slope\_3 (tendance 6s) & 0.20 \\
valeur instantanée & 0.08 \\
v\_lag1 & 0.07 \\
sensor\_idx & 0.06 \\
v\_lag2 -- v\_lag5 & 0.04 -- 0.03 \\
\hline
\end{tabular}
\end{center}

Les deux pentes représentent 51\% du poids total de la décision, confirmant que le modèle détecte les anomalies par leur \textbf{dynamique} (changement) plutôt que par leur valeur absolue.
...
\begin{lstlisting}[language=Python,caption={Fonction d'inference temps reel}]
def predict_and_alert(df, epoch_id):
    model = PipelineModel.load("/home/spark/.ivy2/predictive_model")

    for k in range(1, 6):
        df = df.withColumn(f"v_lag{k}",
            coalesce(lag("valeur", k).over(fenetre), col("valeur")))
    df = df.withColumn("slope_1", col("valeur") - col("v_lag1"))
    df = df.withColumn("slope_3", (col("valeur") - col("v_lag3")) / 3)

    preds = model.transform(df)
    extract_prob = udf(lambda v: float(v[1]), DoubleType())
    preds = preds.withColumn("score_anomalie", extract_prob(col("probability")))

    preds.select("machine_id", "type_capteur", "valeur",
                  "score_anomalie", "timestamp") \
         .write \
         .jdbc(URL, "alertes_predictions", mode="append", properties=PROPS)
\end{lstlisting}
...
En anomalie, le score monte à $>$0.7 dès que la pente positive est détectée (2 à 4~secondes après le début de la dérive). La figure~\ref{fig:score-evolution} illustre la progression typique :

\[
\begin{array}{c|c}
\text{Temps (s)} & \text{Score anomalie} \\ \hline
0 & 0.026 \quad \text{(normal : feuilles quasi-pures)} \\
2 & 0.035 \quad \text{(pente > 0 : quelques arbres voient le changement)} \\
4 & 0.120 \quad \text{(divergence : feuilles partagées)} \\
6 & 0.450 \quad \text{(dérive confirmée : majorité des arbres penchent)} \\
8 & 0.700 \quad \text{(alerte : 21+ arbres dans des feuilles à >80\% d'anomalies)} \\
10+ & 0.700-0.767 \quad \text{(palier : feuilles saturées)}
\end{array}
\]

\subsubsection{Pourquoi les scores plafonnent-ils souvent à \textasciitilde0.7 ?}

Le RandomForestClassifier calcule le score comme la moyenne des distributions de classes dans les feuilles de chaque arbre. Avec des feuilles dont la proportion d'anomalies varie de 0 à 1 selon la région de l'espace des features, le score prend des valeurs continues et non des multiples de $1/30$.

Quand 21~arbres sur 30 orientent la donnée vers des feuilles à forte proportion d'anomalies (0.8--1.0) mais que les 9 autres l'envoient vers des feuilles majoritairement normales (0.3--0.5), la moyenne se stabilise autour de 0.7. Atteindre 0.8 nécessiterait que presque toutes les feuilles soient à $>$0.8 — possible mais plus rare.
...
\hline
0.02 -- 0.06 & Toutes les feuilles sont quasi-pures (normales) \\
0.3 -- 0.5 & Divergence naissante, feuilles partagées \\
\textbf{0.7} & \textbf{Majorité des feuilles penchent vers anomalie} \\
...
Le score affiché (\textit{stat panel} \og Niveau d'anomalie \fg) est la moyenne des distributions de classes issues des feuilles de chaque arbre du RandomForest. Ce n'est \textbf{pas} un simple comptage d'arbres ayant voté \og anomalie \fg{}.

\noindent\textbf{Fonctionnement :} chaque arbre du RandomForest, au lieu de voter binairement (normal/anomalie), renvoie la proportion de classes dans la feuille où arrive la donnée. Par exemple, si une feuille contient 95 échantillons \og normal \fg{} et 5 \og anomalie \fg{} (résidu des données d'entraînement), l'arbre produit $[0.95, 0.05]$. Le score final est la moyenne de ces 30 distributions :

\[
\text{score} = \frac{1}{30} \sum_{i=1}^{30} \text{proportion\_anomalie}(\text{feuille}_i)
\]

En régime normal, chaque feuille contient un petit résidu d'échantillons anormaux (le dataset d'entraînement avait $\sim$8.6\% d'anomalies), donnant une moyenne de $\sim$0.026. Ce n'est ni un risque de 2.6\% ni un arbre sur 30 — c'est simplement la sortie du modèle quand tout est stable.

\begin{itemize}
    \item \textbf{0.000 -- 0.399} (couleur \textcolor{green}{vert}) : fonctionnement nominal. Aucune action requise.
    \item \textbf{0.400 -- 0.699} (couleur \textcolor{#d29922}{jaune}) : dérive détectée. Surveillance renforcée conseillée.
    \item \textbf{0.700 -- 1.000} (couleur \textcolor{red}{rouge}) : anomalie confirmée. Intervention requise.
\end{itemize}

\textbf{Le score n'est pas une mesure de santé globale.} Il ne reflète que les deux capteurs simulés (température et vibration) et ne remplace pas une inspection réelle.
